package com.example.cloneshopee.home.recyclerViewAdapter.menu.giatui
...
import com.example.cloneshopee.home.viewModels.menu.GiatUiViewModel
import com.squareup.picasso.Picasso

class SubmenuGiatUiAdapter(val submenuGiatUiList: ArrayList<SubmenuGiatUiModel>, val giatUiActivity: GiatUiActivity): RecyclerView.Adapter<SubmenuGiatUiAdapter.ViewHolder>() {
    var default_index = 0
    ...
    override fun getItemCount(): Int {
        return submenuGiatUiList.size
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val submenuGiatUiModel: SubmenuGiatUiModel = submenuGiatUiList[position]
        Picasso.get().load(submenuGiatUiModel.IMAGE_URL).resize(48,48).centerCrop().into(holder.imgvIcon)
        holder.txtvName?.text = submenuGiatUiModel.NAME

        holder.itemView.setOnClickListener { view: View ->
            giatUiViewModel = ViewModelProviders.of(giatUiActivity).get(GiatUiViewModel::class.java)
            giatUiViewModel.onPositionChanged(position)
