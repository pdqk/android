...
class HoaActivity : AppCompatActivity() {
    ...
    private var submenuHoaJob = Job()
    private var coroutineSubmenuHoaScope = CoroutineScope(submenuHoaJob + Dispatchers.Main)

    var coroutineSlideImageHoa =
        CoroutineSlideImageHoa()
    var coroutineVoucherHoa =
        CoroutineVoucherHoa()
    var coroutineSubmenuHoa =
        CoroutineAllSubmenuHoa()
    ...
    private fun setupSliderImage(){
        coroutineSlideImageHoa.onCoroutineGetSlideImage(coroutineSlideImageScope, this, haveSubmenuLayoutBinding)
    }

    private fun setupVoucher(){
        coroutineVoucherHoa.onCoroutineGetVoucher(coroutineVoucherScope, haveSubmenuLayoutBinding, this)
    }

    private fun setupSubmenuHoa(){
        coroutineSubmenuHoa.onCoroutineGetSubmenuHoa(coroutineSubmenuHoaScope, haveSubmenuLayoutBinding, this, this)
    }
    ...
    override fun onStop() {
        super.onStop()
        coroutineSlideImageHoa.onCoroutineDone(slideImageJob)
        coroutineVoucherHoa.onCoroutineDone(voucherJob)
        coroutineSubmenuHoa.onCoroutineDone(submenuHoaJob)
    }
